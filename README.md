...
| Hosting | Vercel + Neon branches; GitHub Actions CI |
...
Optional: `ADMIN_EMAILS` (can only *restrict* the hardcoded allowlist), `REPLAN_REQUESTS_PER_HOUR`, `REPLAN_USD_CAP_PER_DAY`, `MAILSLURP_API_KEY` (e2e sign-in).
...
npm run e2e            # playwright (starts the app itself; needs MAILSLURP_API_KEY + a working Resend key)
...
e2e/              Playwright specs + fixtures (real OTP sign-in via MailSlurp)
...
1. Push to `main` → GitHub Actions `CI` (`.github/workflows/deploy.yml`): unit tests → deploy a **tested Vercel preview** pointed at a fresh Neon copy-on-write branch of prod (migrated first) → full Playwright e2e against that exact URL. No auth bypass exists; e2e signs in through the real OTP email. `/api/test/*` fixture endpoints are data-only, hard-off on production, and locked with a per-run HMAC secret.
2. Ship → run **"Promote to production"** (`.github/workflows/promote.yml`) from the Actions tab. It refuses any commit whose CI run isn't green, migrates prod, then builds and deploys that same commit. Vercel's own git auto-deploy is disabled in `vercel.json`, so this workflow is the only path to prod.

`scripts/ship.sh` / `npm run ship` is legacy and bypasses every gate — do not use it.
