...
1. **Open a PR into `main`** → `.github/workflows/ci.yml` (workflow **CI**), re-run on every push to the PR. On a docs-only PR, the preview and both E2E jobs are skipped:
   ...
   If branch protection is turned on, require only **Decide scope** and **Unit tests** — the two jobs that run on every PR — and *"require branches to be up to date before merging"*. The snippet and the reasoning are in `docs/design/deploy-pipeline.md`.
