#!/usr/bin/env bash
# Hand a task brief under .claude/tasks/ (gitignored) to Claude Code as its opening prompt.
#   scripts/claude-task.sh .claude/tasks/2026-09-22-place-names-finish.md
set -euo pipefail
cd "$(dirname "$0")/.."
brief="${1:?usage: scripts/claude-task.sh .claude/tasks/<brief>.md}"
