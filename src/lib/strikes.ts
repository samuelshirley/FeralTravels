...
/**
 * The tier a message was sorted into. Re-exported from `./pennyGate` rather
 * than restated: two definitions of the same three-value union is exactly the
 * kind of drift that ends with one file's `T2` meaning something the other's
 * does not.
 */
export type { MessageTier } from './pennyGate';
