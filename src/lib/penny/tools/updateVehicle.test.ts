 ...
 * safety number (range_km) from chat. That is
 * set only in onboarding and Settings. The bug this locks out: "I'll need
 ...
 * preference and silently rewrote range_km. The schema must make
...
describe('update_vehicle validator', () => {
  it('rejects range_km — range is onboarding/Settings-only', () => {
  ...
  it('strips range fields even when smuggled alongside fuel_type', () => {
    const result = validator(ctx).safeParse({
      data: { fuel_type: 'diesel', range_km: 250 },
    });
