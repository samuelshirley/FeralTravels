...
1. **Open a PR into `main`** → GitHub Actions `CI` workflow (`.github/workflows/ci.yml`), re-run on every push to the PR:
   ...
   - **Mobile typecheck** — `tsc --noEmit` in `mobile/`. Runs alongside Unit tests and does NOT gate the preview (the preview is the web app). Added 2026-08-19 because `.github/workflows/mobile.yml` now publishes `mobile/` over the air on merge — without this the first thing to catch a mobile type error is a tester's phone.

   These are the **required checks** in branch protection.
