...
- **The iOS app moved to a NEW Apple developer account, and a new bundle id (2026-09-02).** `com.feraltravels.ios`, with products `com.feraltravels.ios.monthly` / `.annual`. **The old `com.feraltravels.app` is permanently unusable** — uploading a TestFlight build binds a bundle id to the account that uploaded it, forever (confirmed with Apple DTS), and one was uploaded under the old US team. Nothing had shipped and there are no users, so it was a pure rename.
  ...
  - **Still pointing at the old account, pending the new app record:** `ascAppId` and `appleTeamId` in `mobile/eas.json`, and `EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID` on both build profiles. **The Google one is the quiet failure** — iOS OAuth clients are bound to a bundle id, so the existing client cannot authorise the new one, and a wrong-but-present value shows a sign-in button that dead-ends at the redirect rather than hiding itself the way an empty one does. `AUTH_GOOGLE_IOS_CLIENT_ID` (Vercel, both environments) needs the same new value, and `APPLE_APP_BUNDLE_ID` should be DELETED from Vercel if set — the code default is right now, and a stale env var would silently override it and fail every Apple sign-in. The table in `docs/design/iap-setup.md` is the list.
