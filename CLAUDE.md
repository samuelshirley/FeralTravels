...
**Cost, and why the job is gated.** A simulator needs macOS and GitHub bills those runners at ~10x Linux. The job runs only when something under `mobile/` changed, and caches the built `.app` against a NATIVE fingerprint (`package-lock.json` + `app.config.js` + `eas.json`) — the same OTA-vs-native distinction `scripts/decide-mobile-release.mjs` draws. A JS-only change restores the binary and rebuilds only the bundle. Widening that filter is a deliberate decision, not a cleanup.
