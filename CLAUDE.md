...
**Why not a mailbox:** MailSlurp's free trial expired and can't be renewed (2026-08-14); a Google Workspace mailbox over IMAP then cost an afternoon on app passwords, alias-vs-user and Workspace policy without ever authenticating. Both were third parties who could switch the pipeline off, and did. Eleven of the twelve specs never cared about email — they needed a signed-in user.

**Known coverage hole:** nothing now proves Resend delivers or that the email template contains a parseable code. `e2e/login-otp.spec.ts` is that test and it is SKIPPED. `E2E_MAX_SKIPPED=1` exists for exactly that one spec — a second skip reds CI. The intended fix is Resend inbound on a subdomain (it has a REST API for retrieving received mail), which restores the spec without a third-party inbox.
