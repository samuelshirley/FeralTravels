...
**E2E auth: NO bypass exists (removed 2026-07-02).** Every authenticated spec signs in through the REAL OTP email flow. Since 2026-08-14 the mail side is a **mailbox we own**, not a disposable-inbox service: `e2e@feraltravels.com` (Google Workspace) read over IMAP with an app password, and each test mints a unique **plus-addressed** variant — `e2e+playwright-<runid>-<n>@feraltravels.com`. `server/auth/otp.ts` normalises with `email.trim().toLowerCase()` and nothing strips the `+tag`, so every test still gets a genuinely fresh user; one mailbox gives unlimited ones, free, with no third party able to expire it. (MailSlurp preceded this and its free trial expired 30 days after signup, silently mass-skipping the suite from ~2026-08-01 until 2026-08-14 — see `e2e/fixtures/mailbox.ts`.) Requires `E2E_IMAP_USER` + `E2E_IMAP_PASSWORD` (an APP password, needs 2-Step Verification on the account); specs skip without them, and CI preflights the mailbox (`npm run e2e:preflight`) before the suite so a broken credential fails in seconds instead of as eleven silent skips.
...
E2E cross-spec state: none — each test gets a fresh MailSlurp user and seeds its own canonical graph (`seedCanonicalFixture(email)` in `e2e/fixtures/test-trip.ts`), so specs can't contaminate each other. Cost of the model: 1 real Resend OTP send + an IMAP poll (~5–15s) per authenticated test; if reading a mailbox per test ever becomes the bottleneck, the documented fallback is one fresh user per RUN signed in once in global-setup with a shared Playwright `storageState`.
