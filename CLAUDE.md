...
- **The iOS app moved to a NEW Apple developer account, and a new bundle id (2026-09-02).** `com.feraltravels.ios`, with products `com.feraltravels.ios.monthly` / `.annual`. **The old `com.feraltravels.app` is permanently unusable** — uploading a TestFlight build binds a bundle id to the account that uploaded it, forever (confirmed with Apple DTS), and one was uploaded under the old US team. Nothing had shipped and there are no users, so it was a pure rename.
  ...
  - **`ascAppId` was REMOVED from `mobile/eas.json`, not updated** (2026-09-02). It held `6802705582` — the app record on the old US team, which no longer exists — and leaving it would have aimed `eas submit` at a dead record. Absent is safe: EAS resolves the target from the bundle id and `appleTeamId` (`TJX3F3832H`), or prompts. It gets set from the new record's Apple ID number once that record exists. Note `ascAppId` lives in the `submit` block, so it never affected `eas build`.
